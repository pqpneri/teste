(() => {
  ...
  function profileName() {
    return state.profile?.full_name || state.user?.user_metadata?.full_name || state.profile?.username || '';
  }
  ...
  async function updateOwnProfile(changes) {
    ...
    const username = String(changes.username || '').trim().toLowerCase();
    const email = String(changes.email || '').trim().toLowerCase();
    if (fullName.length < 2) throw new Error('Informe seu nome.');
    if (!/^[a-z0-9._-]{3,30}$/.test(username)) throw new Error('Use um usuário válido com 3 a 30 caracteres.');
    if (!/^\S+@\S+\.\S+$/.test(email)) throw new Error('Informe um e-mail válido.');

    if (state.profile?.username?.toLowerCase() !== username) {
      const { data: exists, error: existsError } = await requireClient().rpc('username_exists_for_other_user', {
    ...
    const authChanges = { data: { ...(user.user_metadata || {}), full_name: fullName, username } };
    if ((user.email || '').toLowerCase() !== email) authChanges.email = email;
    const { data: authData, error: authError } = await requireClient().auth.updateUser(authChanges);
    ...
    state.user = authData.user || user;
    state.profile = profile;
    emit('unigames:profile-updated', profile);
    return profile;
  }
  ...
  window.UnigamesDB = {
    ...
    loadAll, loadProfile, loadUsers, updateUserProfile, updateOwnProfile, saveTransfer, saveCustomer, deleteTransfer, deleteCustomer, clearTransfers
  };
    ...
    try {
      ...
      client = window.supabase.createClient(config.url, config.anonKey, {
        auth: {
          ...
          storageKey: 'unigames-auth-session-v21'
        }
      });
