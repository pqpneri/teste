(() => {
  ...
  function refreshHeaderGreeting(name) {
    const el = $('topbarGreeting');
    if (!el) return;
    el.textContent = greetingFor(name);
  ...
  function showAppMessage(text, type = 'error', options = {}) {
    ...
    if (text && !options.persistent) {
      showAppMessage.timer = setTimeout(() => {
        el.textContent = '';
  ...
  async function handleRegister(event) {
    ...
    const receiveEmails = $('registerEmailNotifications')?.checked !== false;
    const button = $('registerSubmit'); button.disabled = true; button.textContent = 'Criando conta...';
    try {
      const redirectTo = window.UNIGAMES_DATABASE_CONFIG?.emailRedirectTo || window.location.origin + window.location.pathname;
      const { data, error } = await client.auth.signUp({
        email,
        password,
        options: {
          emailRedirectTo: redirectTo,
          data: { full_name: name, role: 'transferencia', email_notifications: receiveEmails }
        }
      });
      if (error) throw error;
      if (data.session && data.user) await unlock(data.user);
      else {
        setMessage('registerMessage', receiveEmails ? 'Conta criada. Enviamos a confirmação e as boas-vindas para seu e-mail.' : 'Conta criada. Confirme seu e-mail para liberar o acesso.', 'success');
        $('registerForm').reset();
      }
