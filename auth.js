(() => {
  ...
  function syntheticEmail(username) { return `${normalizeUsername(username)}@login.unigames.local`; }
  ...
  async function handleRegister(event) {
    ...
    try {
      ...
      if (data.session && data.user) {
        ...
        setMessage('registerMessage', 'Conta criada, mas o acesso automático está bloqueado. Desative a confirmação de e-mail no Supabase Auth.', 'error');
      }
    } catch (error) {
      setMessage('registerMessage', error.message.startsWith('Este usuário') ? error.message : `Não foi possível criar a conta: ${error.message}`);
