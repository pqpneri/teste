(() => {
  ...
  const ROLE_LABELS = { admin: 'Administrador', gestor: 'Gestor', operacional: 'Operacional', comercial: 'Comercial', transferencia: 'Transferência' };
  ...
  function applyPermissions(profile) {
    const role = profile?.role || 'transferencia';
    const allowed = ROLE_ACCESS[role] || ROLE_ACCESS.transferencia;
  ...
  function accessChips(role) {
    return (ROLE_ACCESS[role] || ROLE_ACCESS.transferencia)
  ...
  function render() {
    ...
    const filtered = users.filter(u => (!q || `${u.full_name || ''} ${u.username || ''}`.toLowerCase().includes(q)) && (!role || u.role === role));
    updateMetrics(filtered);
    const list = $('usersList'); if (!list) return;
    list.innerHTML = '';
    filtered.forEach(user => {
      const roleKey = user.role || 'transferencia';
      const card = document.createElement('article');
      card.className = 'user-card-v10';
      card.innerHTML = `
        <div class="user-main-v10">
          <div class="user-avatar">${escapeHtml(initials(user.full_name || user.username))}</div>
          <div class="user-info"><strong>${escapeHtml(user.full_name || 'Sem nome')}</strong><span>@${escapeHtml(user.username || 'sem-usuario')}</span><small>Criado em ${new Date(user.created_at).toLocaleDateString('pt-BR')}</small></div>
        </div>
  ...
  function openEditor(user) {
    ...
    $('editUserEmail').value = user.username ? '@' + user.username : '';
    $('editUserRole').value = user.role || 'transferencia';
  ...
  document.addEventListener('unigames:authenticated', async event => {
    ...
    if (profile?.is_active === false) {
      window.showAppMessage?.('Esta conta está bloqueada. Procure um administrador.', 'error', { persistent: true });
      const loginMessage = document.getElementById('loginMessage');
      if (loginMessage) { loginMessage.textContent = 'Esta conta está bloqueada. Procure um administrador.'; loginMessage.className = 'login-message error'; }
