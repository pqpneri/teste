...
function saveReport(report){const history=getHistory();history.unshift(report);saveHistory(history);renderHistory();window.UnigamesDB?.saveTransfer(report)}
...
function deleteReport(id){if(!confirm('Deseja excluir este relatório do histórico?'))return;saveHistory(getHistory().filter(item=>item.id!==id));renderHistory();window.UnigamesDB?.deleteTransfer(id)}
...
document.getElementById('clearHistoryButton').addEventListener('click',()=>{if(!getHistory().length)return;if(confirm('Deseja apagar todo o histórico de relatórios? Esta ação não poderá ser desfeita.')){localStorage.removeItem(STORAGE_KEY);renderHistory();window.UnigamesDB?.clearTransfers()}});
...
(()=>{
  ...
  const newCode=()=>{const chars='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';code='UNI-'+Array.from({length:4},()=>chars[Math.floor(Math.random()*chars.length)]).join('');$('customerCode').textContent=code;$('previewCode').textContent=code};
  ...
  function saveCustomerReport(d, action='generated'){
    ...
    if(index>=0) list[index]={...list[index],...report,createdAt:list[index].createdAt||report.createdAt};
    else list.unshift(report);
    localStorage.setItem(key,JSON.stringify(list));
    window.UnigamesDB?.saveCustomer(report);
  ...
  function canvas(d){
    const w=620,itemH=76,base=360,h=base+d.products.length*itemH;
    const cv=document.createElement('canvas');cv.width=w;cv.height=h;const c=cv.getContext('2d');
    c.fillStyle='#eef2f8';c.fillRect(0,0,w,h);
    c.shadowColor='rgba(15,28,67,.18)';c.shadowBlur=28;c.shadowOffsetY=12;rr(c,18,18,w-36,h-36,30,'#ffffff');c.shadowColor='transparent';
    const head=c.createLinearGradient(18,18,w-18,160);head.addColorStop(0,'#061633');head.addColorStop(.58,'#173d91');head.addColorStop(1,'#6548da');
    c.save();c.beginPath();c.roundRect(18,18,w-36,142,[30,30,0,0]);c.clip();c.fillStyle=head;c.fillRect(18,18,w-36,142);
    c.fillStyle='rgba(246,199,105,.28)';c.beginPath();c.arc(540,5,105,0,Math.PI*2);c.fill();
    c.fillStyle='rgba(39,199,231,.20)';c.beginPath();c.arc(35,150,95,0,Math.PI*2);c.fill();c.restore();
    c.fillStyle='#fff';c.font='900 26px Arial';c.fillText('UNIGAMES',42,58);c.font='800 9px Arial';c.globalAlpha=.68;c.fillText('PROPOSTA DIGITAL',42,79);c.globalAlpha=1;
    rr(c,430,40,145,58,14,'rgba(4,15,42,.23)','rgba(255,255,255,.22)');c.fillStyle='#dfe5ff';c.font='800 8px Arial';c.fillText('CÓDIGO',446,61);c.fillStyle='#ffe39b';c.font='900 16px Arial';c.fillText(d.code,446,84);
    c.fillStyle='#fff';c.beginPath();c.ellipse(310,150,335,32,0,0,Math.PI*2);c.fill();
    rr(c,40,117,58,58,17,'#f3bb70','#ffffff');c.fillStyle='#49345d';c.font='900 16px Arial';c.textAlign='center';c.fillText(initials(d.name),69,152);c.textAlign='left';
    c.fillStyle='#6d54d4';c.font='900 8px Arial';c.fillText((d.type||'ATENDIMENTO').toUpperCase(),114,128);c.fillStyle='#161b35';c.font='900 18px Arial';wrap(c,(d.name||'NOME DO CLIENTE').toUpperCase(),114,151,430,21,1);c.fillStyle='#80879a';c.font='11px Arial';c.fillText(`${d.phone||'Telefone não informado'}  •  ${d.date}`,114,170);
    c.fillStyle='#858ca0';c.font='900 8px Arial';c.fillText(`PRODUTOS  •  ${d.products.length} ${d.products.length===1?'ITEM':'ITENS'}`,42,207);
    let y=220;
    d.products.forEach(p=>{
      rr(c,42,y,536,62,14,'#f9faff','#e7ebf4');
      c.fillStyle='#202641';c.font='800 12px Arial';wrap(c,p.product||'Produto não informado',56,y+21,350,15,1);
      c.fillStyle='#81889a';c.font='9px Arial';c.fillText(`${p.quantity} un. × ${money(p.unit)}  •  ${p.paymentLabel||'Pagamento não informado'}`,56,y+43);
      c.fillStyle='#4c4fd2';c.font='900 13px Arial';c.textAlign='right';c.fillText(money(p.subtotal),562,y+34);c.textAlign='left';y+=itemH;
    });
    const total=c.createLinearGradient(42,y,578,y+74);total.addColorStop(0,'#0b69d0');total.addColorStop(.58,'#1686ef');total.addColorStop(1,'#3fa9ff');rr(c,42,y,536,74,18,total);
    c.fillStyle='#dce3ff';c.font='900 8px Arial';c.fillText('VALOR TOTAL',58,y+24);c.fillStyle='#fff';c.font='900 27px Arial';c.fillText(money(d.total),58,y+55);c.fillStyle='#ffe39b';c.font='16px Arial';c.textAlign='right';c.fillText('✦',555,y+28);c.textAlign='left';
    y+=88;c.fillStyle='#9399aa';c.font='900 8px Arial';c.fillText('LOJA',44,y+14);c.fillText('CANAL',342,y+14);c.fillStyle='#303650';c.font='800 10px Arial';c.fillText(d.store||'Não informada',44,y+32);c.fillText(d.channel||'Não informado',342,y+32);
    return cv
  }
  const toBlob=cv=>new Promise(r=>cv.toBlob?r(cv.toBlob(r,'image/png',1)):r(null));
  async function generate(){const d=data();saveCustomerReport(d,'image');const cv=canvas(d);dataUrl=cv.toDataURL('image/png');blob=await new Promise(r=>cv.toBlob(r,'image/png',1));filename=`atendimento-${d.code}.png`;img.src=dataUrl;dialogMsg.textContent='Imagem gerada. Escolha como salvar.';dialog.showModal();show('Cartão gerado com o novo layout.','success')}
...
(()=>{
  ...
  function remove(id){if(!confirm('Excluir este relatório de cliente?'))return;localStorage.setItem(key,JSON.stringify(get().filter(r=>r.id!==id)));window.UnigamesDB?.deleteCustomer(id);render()}
  function render(){
...
// Sincroniza o banco remoto com o cache local ao abrir o sistema.
window.addEventListener('unigames:database-ready', async () => {
  try {
    await window.UnigamesDB.syncAll();
    renderHistory();
    window.dispatchEvent(new CustomEvent('customerReportsUpdated'));
  } catch (error) {
