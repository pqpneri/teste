...
function validateForm(messageMode = false) {
  if (!form.checkValidity()) {
    form.reportValidity();
    showMessage(messageMode
      ? 'Preencha todos os campos obrigatórios para gerar o relatório.'
      : 'Preencha todos os campos obrigatórios.', 'error');
    return false;
  }
...
form.addEventListener('reset', () => setTimeout(() => showMessage(''), 0));
...
document.getElementById('exportButton').addEventListener('click', () => {
  ...
  const headers = ['Data', 'Loja de Origem', 'Loja Atual', 'Produto', 'Quantidade', 'Transporte', 'Motivo'];
  const rows = transfers.map(item => [
    formatDate(item.createdAt), item.lojaOrigem, item.lojaAtual, item.produto,
    item.quantidade, item.transporte, item.motivo
  ]);
...
function buildWhatsappReport() {
  return [
    ...
    '',
    `${quantidade.value.trim()} - ${produto.value.trim().toUpperCase()}`,
    '',
