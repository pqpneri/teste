...
(()=>{
  ...
  function canvas(d){
    const itemH=84, w=720, h=410+d.products.length*itemH;
    const cv=document.createElement('canvas');cv.width=w;cv.height=h;const c=cv.getContext('2d');
    c.fillStyle='#f2f2f8';c.fillRect(0,0,w,h);
    const outer=c.createLinearGradient(0,0,w,h);outer.addColorStop(0,'#171b49');outer.addColorStop(.55,'#4a2f91');outer.addColorStop(1,'#087d9f');
    rr(c,20,20,w-40,h-40,30,'#ffffff');
    c.save();c.beginPath();c.roundRect(20,20,w-40,132,[30,30,0,0]);c.clip();c.fillStyle=outer;c.fillRect(20,20,w-40,132);c.fillStyle='rgba(255,224,138,.22)';c.beginPath();c.arc(620,15,130,0,Math.PI*2);c.fill();c.restore();
    c.fillStyle='#fff';c.font='italic 900 29px Arial';c.fillText('UNIGAMES',48,64);c.font='800 10px Arial';c.globalAlpha=.72;c.fillText('PROPOSTA DIGITAL',48,86);c.globalAlpha=1;
    rr(c,500,45,164,60,15,'rgba(255,255,255,.12)','rgba(255,255,255,.25)');c.fillStyle='#dedfff';c.font='800 9px Arial';c.fillText('CÓDIGO',518,67);c.fillStyle='#ffe08a';c.font='900 18px Arial';c.fillText(d.code,518,91);
    c.fillStyle='#f6c879';c.beginPath();c.arc(70,185,28,0,Math.PI*2);c.fill();c.fillStyle='#35245f';c.font='900 15px Arial';c.textAlign='center';c.fillText(initials(d.name),70,191);c.textAlign='left';
    c.fillStyle='#6f55c5';c.font='900 9px Arial';c.fillText(d.type.toUpperCase(),112,167);c.fillStyle='#202448';c.font='900 20px Arial';wrap(c,(d.name||'NOME DO CLIENTE').toUpperCase(),112,191,500,23,1);c.fillStyle='#858aa0';c.font='12px Arial';c.fillText(`${d.phone||'Telefone não informado'} • ${d.date}`,112,215);
    c.fillStyle='#777c94';c.font='900 9px Arial';c.fillText(`PRODUTOS • ${d.products.length} ${d.products.length===1?'ITEM':'ITENS'}`,48,256);
    let y=274;
    d.products.forEach(p=>{
      rr(c,48,y,624,70,14,'#f8f7ff','#ecebfd');
      c.fillStyle='#262a51';c.font='800 13px Arial';wrap(c,p.product||'Produto não informado',64,y+23,420,17,1);
      c.fillStyle='#7c8198';c.font='10px Arial';c.fillText(`${p.quantity} un. × ${money(p.unit)} • ${p.paymentLabel||'Pagamento não informado'}`,64,y+47);
      c.fillStyle='#4f3dad';c.font='900 14px Arial';c.textAlign='right';c.fillText(money(p.subtotal),652,y+37);c.textAlign='left';y+=itemH;
    });
    const tg=c.createLinearGradient(48,y,672,y+82);tg.addColorStop(0,'#171b49');tg.addColorStop(.65,'#4b2f91');tg.addColorStop(1,'#087d9f');rr(c,48,y,624,82,18,tg);c.fillStyle='#dadcff';c.font='900 9px Arial';c.fillText('VALOR TOTAL',66,y+27);c.fillStyle='#fff';c.font='900 29px Arial';c.fillText(money(d.total),66,y+61);c.fillStyle='rgba(255,255,255,.75)';c.font='10px Arial';c.textAlign='right';c.fillText(d.products.map(p=>p.paymentLabel).filter(Boolean)[0]||'Condição conforme os itens',650,y+51);c.textAlign='left';y+=99;
    c.fillStyle='#888da2';c.font='900 8px Arial';c.fillText('LOJA',50,y+18);c.fillText('CANAL',390,y+18);c.fillStyle='#343959';c.font='800 11px Arial';c.fillText(d.store||'Não informada',50,y+38);c.fillText(d.channel||'Não informado',390,y+38);
    return cv
  }
