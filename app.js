...
async function deleteReport(id){if(!confirm('Deseja excluir este relatório do histórico?'))return;try{await window.UnigamesDB.deleteTransfer(id);renderHistory()}catch(error){alert('Não foi possível excluir: '+error.message)}}
...
(()=>{
  ...
  async function remove(id){if(!confirm('Excluir este relatório de cliente?'))return;try{await window.UnigamesDB.deleteCustomer(id);render()}catch(error){alert('Não foi possível excluir: '+error.message)}}
  function render(){
